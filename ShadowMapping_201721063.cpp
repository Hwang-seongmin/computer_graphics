...
void init() {
	...
	// Path location in the model
	loadJ3A("/Users/USER/source/repos/ShadowMapping_201721063/ShadowMapping_201721063/apple.j3a");

	int w, h, n;
	stbi_set_flip_vertically_on_load(true);
	void* buf = stbi_load(("/Users/USER/source/repos/ShadowMapping_201721063/ShadowMapping_201721063/" + diffuseMap[0]).c_str(), &w, &h, &n, 4);
	...
	buf = stbi_load(("/Users/USER/source/repos/ShadowMapping_201721063/ShadowMapping_201721063/" + bumpMap[0]).c_str(), &w, &h, &n, 4);
...
void render(GLFWwindow* win) {
	...
	mat4 modelMat = mat4(1); //glm::rotate(theta,glm::vec3(0,1,0));
	...
	vec3 cameraPos = vec3(rotate(theta, vec3(0, 1, 0)) * rotate(phi, vec3(1, 0, 0)) * vec4(cameraPosition * cameraDistance, 1));

	mat4 viewMat = lookAt(cameraPos, vec3(0, 0, 0), vec3(0, 1, 0));
	mat4 projMat = perspective(fovy * PI / 180, w / (float)h, 0.1f, 100.0f);
